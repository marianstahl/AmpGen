...
void EventList::loadFromTree( TTree* tree, const ArgumentPack& args )
{
  ...
  if(weightBranch.empty()) temp.setWeight( 1 );
  if(!getGenPdf) temp.setGenPdf( 1 );
  ...
  if( weightBranch != "" ){
    ...
    m_extensions[weightBranch] = pos++;
  }
  if( filter != "" ){
    if( entryList.size() != 0 ){
      WARNING("Specified entry list and filter, will overwrite list with specified selection");
    }
    if(!weightBranch.empty()) WARNING("Specified filter and weight branch. Is this really what you want to do?");
    tr.prepare();
